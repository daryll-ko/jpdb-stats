...
# ╔═╡ 331b8ff4-e11d-4ae5-abaf-b3686cd388f8
md"# `jpdb.io` stat visualizer"

# ╔═╡ 88d99abe-a84f-40e6-9519-5706d52138f0
md"You'll need your `reviews.json` file from jpdb's `Settings` page:"
...
# ╔═╡ cf40e908-dd6e-4044-8fa5-f050477aafd7
md"### Structs"
...
# ╔═╡ 6f591a8c-0bfc-45d0-adf3-41d2e0bdad92
get_timestamp(review) = review.timestamp

# ╔═╡ 829db4d1-243b-405e-a1c9-9e87adb7f9c2
get_grade(review) = review.grade
...
# ╔═╡ 0e1b94c5-f926-4977-91c6-df12771b79c1
get_spelling(card) = card.spelling

# ╔═╡ 23951e04-e61f-45d1-99ab-76e915abe789
get_reading(card) = card.reading

# ╔═╡ de45c921-13d0-4789-b582-a67c66e36e86
get_reviews(card) = card.reviews

# ╔═╡ 83d5b194-08d4-4527-8f50-13c939780547
md"### Constants"

# ╔═╡ fef48d1d-eb73-4084-9566-ce765a85b40e
kanji_regex = r"[一-龯]"
...
jouyou_kanji = Set(string.(collect(jouyou_kanji_string)))
...
function parse_review(review)
    timestamp = (localize ∘ unix2datetime)(review["timestamp"])
    grade = review["grade"]
    return Review(timestamp, grade)
end
...
function get_kanji(spelling)
	return eachmatch(kanji_regex, spelling) |>
			matches -> map(match -> match.match, matches) |>
			unique
end
...
function get_kanji_counter(cards)
    kanji_counter = Dict()
    for card in cards
		kanji = get_kanji(card.spelling)
		for c in kanji
			kanji_counter[c] = get(kanji_counter, c, 0) + 1
		end
    end
...
jouyou_kanji_not_known = setdiff(jouyou_kanji, kanji_known)

# ╔═╡ 1e24c261-f1bc-464e-b9c0-7b8cbf2563fa
non_jouyou_kanji_known = setdiff(kanji_known, jouyou_kanji)

# ╔═╡ 14502756-a1c3-41b4-be3f-5cc0b37a51bd
num_days = length(date_counter)
...
begin
	...
	ys = map(
		...
		sort(grades_to_show, by = g -> grade_to_order[g])
	)

	cur_total = length(ys) > 0 ?
					ys .|> first |> sum :
					zeros(Int, num_days)
...
# ╔═╡ 277dde37-de15-446d-8f3e-735c566daa8c
num_cards = length(cards)

# ╔═╡ 9b415913-d09a-4c3c-b167-45061c3827ff
num_reviews = length(reviews)

# ╔═╡ 2d3da06a-597d-4493-a1e2-80e8069407b0
num_kanji_known = length(kanji_known)

# ╔═╡ cef1d922-8c7f-4ae4-aaab-cd496b1b5e9d
num_jouyou_kanji_known = length(jouyou_kanji_known)

# ╔═╡ 4a939eb0-f9e8-452f-bb12-16ebcc27572c
percent_jouyou_kanji_known =
	round(100 * num_jouyou_kanji_known / length(jouyou_kanji); digits = 2)
...
# ╔═╡ de881f61-370c-4eee-8838-a94671c232ff
num_jouyou_kanji_not_known = length(jouyou_kanji_not_known)

# ╔═╡ 2ef28e10-2e5e-484b-9de1-a6e5fc1a5f48
num_non_jouyou_kanji_known = length(non_jouyou_kanji_known)
